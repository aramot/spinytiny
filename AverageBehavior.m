...
subplot(6,6,[1:3, 7:9]); 
flex_plot(1:14,rewards(animalstouse,:),'parametric','b',4); xlim([0 15]); ylabel('% Rewarded'); xlabel('Session');
set(gca, 'XTick', 1:14)
subplot(6,6,[4:6, 10:12]); 
reactplot = flex_plot(1:14, ReactionTime(animalstouse,:),'parametric', 'k',4); xlim([0 15]); ylabel('Time (s)'); xlabel('Session');
hold on; c2rplot = flex_plot(1:14, CuetoReward(animalstouse,:), 'parametric','r',4);
legend([reactplot, c2rplot], {'Reaction Time', 'Cue to Reward'});
set(gca, 'XTick', 1:14, 'YTick', 1:14)
FilteredCorrMat = MovementCorrelation(:,:,animalstouse);
subplot(6,6,[13:15, 19:21, 25:27, 31:33]); imagesc(nanmean(FilteredCorrMat, 3))
...
for i = 1:size(FilteredCorrMat,3)
    within(i,1:14) = diag(FilteredCorrMat(:,:,i));
end
for i = 1:size(FilteredCorrMat,3)
    across(i,1:13) = diag(FilteredCorrMat(:,:,i),1);
end
