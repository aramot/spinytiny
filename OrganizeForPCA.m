function [PCAdata] = OrganizeForPCA(varargin)
...
for i = 1:length(varargin)
    ...
    datafiles = fastdir('E:\ActivitySummary',[varargin{i}, '\w*[^Poly]Summary']);    %%% Find any files in the Activity Summary directory that contain the animal name, any number of word characters (usually date), then "Summary", and exclude those that contain the phrase "poly";
   
        load(datafiles{2})

    
    cd 'C:\Users\Komiyama\Desktop\Output Data';
    lookforcorrfiles = cellfun(@(x) strcat(x, '_Correlations'), animalnames, 'uni', false);
    lookforstatclassfiles = cellfun(@(x) strcat(x, '_StatClass'), animalnames, 'uni', false);

    for j = 1:length(lookforcorrfiles)
        corrfiles(j) = fastdir('C:\Users\Komiyama\Desktop\Output Data',lookforcorrfiles{j});
        statclassfiles(j) = fastdir('C:\Users\Komiyama\Desktop\Output Data',lookforstatclassfiles{j});
    end

        load(corrfiles{1})
        load(statclassfiles{1})
    
    eval(['data = ', datafiles{2}(1:end-4)])
    eval(['correlations = ', corrfiles{1}(1:end-4)])
    eval(['statclass = ', statclassfiles{1}(1:end-4)])
    ...
    PCAdata(:,feature) = (DistfromMaxM-nanmean(DistfromMaxM))/nanstd(DistfromMaxM);
        varlist{1,feature} = 'DisttoMaxCorrMove';
        feature = feature+1;
