% function TrialActivityAnalysis(varargin)
% 
% animalnames = varargin;
% lookforfiles = cellfun(@(x) strcat(x, '_TrialInformation'), animalnames, 'uni', false);
% 
...
%     load(files{i});
%     eval(['Data = ', files{i}(1:end-4)])
%     for j = 1:length(Data)
%         if ~isempty(Data{j})
%             for k = 1:length(Data{j})
%                 
%             end
%         end
%     end
...
function [trialdata, Features] = TrialActivityAnalysis(varargin)
...
startwindow = 30;   %%% Time (in frames) to subtract from the initiation of movement for inspection of beginning of trace
stopwindow = 90;    %%% Time (in frames) to add to the initiation of movement for inspection of end of trace
...
Features.ClusteredSpines = sessionClusteredSpines;
Features.ClusteredSpinesbyAnimal = ClusteredSpinesByAnimal;
Features.StatSpinesbyAnimal = StatSpinesByAnimal;
trialdata.TrialAverageAll = trialaverage;
trialdata.TrialAverageByAnimal = trialaveragebyanimal;
trialdata.TrialDendriteAverageAll = trialdendriteaverage;
trialdata.TrialDendriteAverageByAnimal = trialdendriteaveragebyanimal;
Features.MovementLengthDistribution = MovementLengthDistribution;
