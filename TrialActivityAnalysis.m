    ...
    for session = 1:length(currentfile)
        ...
        if ~isempty(currentfile{session})
            if length(currentfile{session})<20
                ...
                numspines(sample, session) = size(currentfile{session}{trialscounted(1)}.trialdendsubactivity,1);        %%% Usually can count on data at session 20 (random, and can be changed according to need)
                if sample > 1
...
%                             MovementEnd = currentfile{session}{trial}.MovementEnd;
                            ...
                            alignedtomovement{1,session}(1:numspines(sample,session),ChosenStart-MovementStart+centermovement:MovementEnd-MovementStart+centermovement,trial) =  currentfile{session}{trial}.trialdendsubactivity(1:numspines(sample,session), ChosenStart:MovementEnd);
