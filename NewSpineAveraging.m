...
for i = 1:length(experimentnames)
    ...
    NewSpinesCorrwithNearbyEarlyMRSs{i} = cell2mat(horzcat(currentdata.NewSpinesCorrwithNearbyEarlyMRSs{:}));
    ...
    ClusteredMoveSpineDeltaFrequency{i} = cell2mat(currentdata.ClusteredMoveSpineDeltaFrequency');
    ...
    ClusteredMoveSpineDeltaAmplitude{i} = cell2mat(currentdata.ClusteredMoveSpineDeltaAmplitude);
...
PofA = PersistentMRSbyanimal(ind)./(NumberofEarlyMovementRelatedSpines(ind)+NumberofMidSessionMovementRelatedSpines(ind));                                                 %%% The probability of a MRS from early sessions being PERSISTENT
...
% xtickangle(gca, 45)
% ylabel('PC1 Score')
% title('Comparison of PC1 scores for Movement Classes')
% 
% maxline = max([nanmedian(datamat{1}), nanmedian(datamat{2})]);
% plot(1:2, (maxline+0.01)*ones(1,2), 'k', 'Linewidth', 2)
% 
...
figure; subplot(1,2,1)
datamat = [{clustPC1scoresbyanimal}, {otherPC1scoresbyanimal}, {clustPC2scoresbyanimal}, {otherPC2scoresbyanimal}];
...
subplot(1,2,2)
datamat = [{clustPC1scoresbyfield}, {otherPC1scoresbyfield}, {clustPC2scoresbyfield}, {otherPC2scoresbyfield}];
