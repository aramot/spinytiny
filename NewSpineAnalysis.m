        ...
        if ~isempty(NewSpines{f})    %%% If there are new spines, find out whether they are close to a nearby movement spine, have a highly correlated partner, etc.
            ...
            if ~isempty(AllMovementSpinesOnEarlySession)
                for ns = 1:length(NewSpines{f})
                    ...
                    ClusteredMoveSpineList = []; ClusteredNewSpineList = []; NearbyMRSlist = [];
...
%                         end
                        ...
                        if NewSpinestoEarlyMovementSpines(1,count)<clusterdistance
                            ...
                            ClusteredNewSpineList = NewSpines{f}(ns);
                            NumberofClusteredMoveSpines = NumberofClusteredMoveSpines+1;
            ...
            TaskCorrelationofClusteredNewSpines{f} = behaviorcorrdataearly(:,unique(ClusteredNewSpineList));
...
%                     end
            ...
            CoActiveHCPMovementCorr = []; OtherMovementCorr= [];
            %%%%%%%%%%%%%%%%%%%%%%%%%% End initialization for HCP section
            highcorrcount = 1;
            for hcp = 1:length(NewSpineMaxInd) %%% Not: every new should should have a corresponding max correlated partner, so you can use "hcp" as an index for new spines;
