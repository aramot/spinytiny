...
%     targetbehavioralsessions = most_stereotyped_session;
    ...
    for f = 1:NumFields
        ...
        %%%%%%%%%%%%%%%%
        %==================================================================
        FocusOnWhichMovements = 'CuedRewarded';    %%% Change this value between "All", "CuedRewarded", and "Rewarded" to consider different categories of movements;
        %==================================================================
...
%         binarizedleverLate(locs) = 0;
        ...
        %%% cue starts (otherwise you can have variable movement lenghts
...
%                 end
            ...
            case 'Ignore'
                eval(['AllEarlySpineBinarizedAligned = ', currentanimal, '_Aligned{', num2str(latesession), '}.BinarizedOverallSpineData;'])
                ...
                for ns = 1:length(NewSpines{f})                             %%% FOR EACH NEW SPINE THAT IS ON A DENDRITE WITH AT LEAST ONE MOVEMENT RELATED SPINE
                    ...
                    for ms = 1:length(MovementSpinestoCompare)  %%% FOR ALL MOVEMENT RELATED SPINES THAT FIT THE ABOVE CRITERIA (i.e. THOSE THAT ARE ON A DENDRITE WITH NEW SPINES)
                        ...
                        switch AnalysisType
                            ...
                            case 'Exclude'
                                eval(['MoveSpineActivityAligned = ', currentanimal, '_Aligned{',num2str(latesession),'}.ProceessedSpineActivity(', num2str(MovementSpinestoCompare(ms)), ',:);'])
...
%                                 window = windowcycle(Zmaxcol); corr_thresh = threshcycle(Zmaxrow);
                            ...
                            if use_dFoF_not_binarized
%                                 window = 6; corr_thresh = 0.25; %%% Values taken from the median values used to optimize the difference between movement activity and shuffled activity
                                for sp = 1:length(comparisonspinestouse)
                                    highcorrwindows = FindHighCorrRegions(MoveSpineActivityAligned,DistanceMatchedActivity(sp,:), ImagingFrequency,window, corr_thresh);
                                    binaryMRSDistMatchedcoactivity(sp,:) = binaryMRSDistMatchedcoactivity(sp,:) | highcorrwindows';
                                end
...
%                                     contingA = analogcoactivetrace>coactivethresh(prct_count);
                                    ...
                                    contingB = NewSpineActivityAligned>0.5;
                                    contingC = MoveSpineActivityAligned>0.5;
...
%                                 contingA = analogcomp_coactivetrace>staticthresh(chosen_address);
                                contingB = AllOtherSpineBinarizedAligned(CompSpFreqMatchedtoNS,:)>0.5;
                                contingC = AllOtherSpineBinarizedAligned(CompSpFreqMatchedtoCMRS,:)>0.5;
...
%                                 threshtouse = min(coactivethresh, staticthresh(chosen_address));
                                ...
                                contingB = MoveSpineActivityAligned>0.5;
                                contingC = DistanceMatchedActivity>0.5;
...
%                             MRSDMTrace = binaryMRSDistMatchedcoactivity;
                            ...
                            if any(CoATrace)
                                ...
                                DotProdofNSDMCoActivePeriodswithMovement(closecount) = NaN;
                                    ChanceNSDMCoActivitywithMovement(closecount) = NaN;
...
%                         DistanceMatchedNonMRPartners = setdiff(DistanceMatchedPartners, MRSsClusteredwiththisNS);
    ...
    %                                 window = 6; corr_thresh = 0.25; %%% Values taken from the median values used to optimize the difference between movement activity and shuffled activity
                            for sp = 1:length(comparisonspinestouse)
                                highcorrwindows = FindHighCorrRegions(NewSpineActivityAligned,DistanceMatchedActivity(sp,:), ImagingFrequency,window, corr_thresh);
                                binaryNSDistMatchedcoactivity(sp,:) = binaryNSDistMatchedcoactivity(sp,:) | highcorrwindows';
                            end
                            move_centric_distmatched_activity = DistanceMatchedActivity(:,cell2mat(frames_during_targetperiods));
                        else
                            move_centric_distmatched_activity = DistanceMatchedBinarized.*binarizedleverLate';
...
%                         NSDMTrace = binaryNSDistMatchedcoactivity;
                        ...
                        DotProdofNSDMCoActivePeriodswithMovement(count) = nanmedian(NSDM_activity);
                        ChanceNSDMCoActivitywithMovement(count) = nanmedian(NSDMshuffled_activity(:));
                    end
                    %==============================================
                    count = 1;
                    for shuff = 1:shuffnum
                    ...
                    [~, ClustwithHighestCorrelation{ns}] =  nanmax(NewSpinesCorrwithCloseEarlyMRS);
                    AllClusterCorrelationsbyNewSpine{f}{ns} = NewSpinesCorrwithCloseEarlyMRS;
                    HighCorrClusters{ns} = find(NewSpinesCorrwithCloseEarlyMRS > 0);
                    NewSpinesCorrwithNearbyEarlyMRSs{f}(ns) = NewSpinesCorrwithCloseEarlyMRS(ClustwithHighestCorrelation{ns});
                    ...
                    %%%
                    [~, ClustwithHighestNoiseCorrelation{ns}] = nanmax(move_centric_cluster_correlation);
                    HighNoiseCorrClusters{ns} = find(move_centric_cluster_correlation>0);
                    MoveCentricClusterCorrelation{f}{ns} = move_centric_cluster_correlation(ClustwithHighestNoiseCorrelation{ns});
                    FailureCentricClusterCorrelation{f}{ns} = failure_centric_cluster_correlation(ClustwithHighestNoiseCorrelation{ns});
                    MoveCentricFrequencyMatchedCorrelation{f}{ns} = MoveCentricFreqMatchedCorrelation(ClustwithHighestNoiseCorrelation{ns});
                    MoveCentricDistanceMatchedCorrelation{f}{ns} = MoveCentricDistMatchedCorrelation;
                    MoveCentricDistanceMatchedCorrelationforMRS{f}{ns} = MoveCentricDistMatchedCorrelationforMRS(ClustwithHighestNoiseCorrelation{ns});
                    %%%
                    ...
                    DotProductofCoActivePeriodsDuringMovement{f}{ns} = DotProdofCoActivePeriodswithMovement(HighCorrClusters{ns});
                    DotProductofFMCoActivePeriodsDuringMovement{f}{ns} = DotProdofFreqCompCoActivePeriodswithMovement(HighCorrClusters{ns});
                    DotProductofNSDMCoActivePeriodsDuringMovement{f}{ns} = DotProdofNSDMCoActivePeriodswithMovement(HighCorrClusters{ns});
                    DotProductofMRSDMCoActivePeriodsDuringMovement{f}{ns} = DotProdofMRSDMCoActivePeriodswithMovement(HighCorrClusters{ns});
                    DotProductofCoActivePeriodsDuringCRMovement{f}{ns} = DotProdofCoActivePeriodswithCRMovement(HighCorrClusters{ns});
                    ...
                    ChanceLevelofCoactivityMovementOverlap{f}{ns} = ChanceCoActiveMovementOverlap(HighCorrClusters{ns});
                    ChanceLevelofFMCoActivitywithmovement{f}{ns} = ChanceFreqMatchedCoActivitywithMovement(HighCorrClusters{ns});
                    ChanceLevelofNSDMCoActivitywithMovement{f}{ns} = ChanceNSDMCoActivitywithMovement(HighCorrClusters{ns});
                    ChanceLevelofMRSDMCoActivitywithMovement{f}{ns} = ChanceMRSDMCoActivitywithMovement(HighCorrClusters{ns});
                    ChanceLevelofCoActivityCRMovementOverlap{f}{ns} = ChanceCoActiveCRMovementOverlap(HighCorrClusters{ns});
                    ChanceLevelofFMCoActivityCRMovementOverlap{f}{ns} = ChanceCompCoActiveCRMovementOverlap(HighCorrClusters{ns});
                end
                %==========================================================
                ...
                for p = prctvals
                    ...
                    %% indicate this here:
                    FilteredClusts = ClustwithHighestCorrelation;
...
%                     ChosenClusterAddresses{prct_count} = unique(cell2mat(horzcat(CoActiveDuringMovement_Addresses{prct_count}{:})')); %%% If you want to use all cluster co-activity as representative 
                ...
                %==========================================================
                movements_to_exclude = [AllClusterCoActivePeriods];  %%% Decide carefully! Can exclude just cluster co-activity, or any movements that contain any activity from either spine
...
%                         threshtouse = min(coactivethresh, staticthresh(chosen_address));
                ...
                prct_count = 1;
                for p = prctvals
                    movements_to_exclude = ChosenClusterAddresses{prct_count};  %%% Decide carefully! Can exclude just cluster co-activity, or any movements that contain any activity from either spine
