...
for animal = 1:length(varargin)-1
    ...
    secondspremovement = 0.5;
    secondspostmovementstart = 2.5;
    TimingValues.SecondsPreMovement = secondspremovement; 
    TimingValues.SecondsPostMovement = secondspostmovementstart;
...
%     xpad = [repmat(ModelMovement(1),1,500),ModelMovement, repmat(ModelMovement(end),1,500)];
    modmov_shifted = ModelMovement-nanmedian(ModelMovement);    %%% Centering the movement around zero reduces edge artifacts of resampling
...
%     ModelMovement = ModelMovement(16:end-16);
    ...
    for f = 1:NumFields
        ...
        %%%%%%%%%%%%%%%%
        FocusOnWhichMovements = 'Rewarded';    %%% Change this value between "Rewarded" and "All" to consider different categories of movements;
        ...
        switch AnalysisType
            ...
            case 'Ignore'
                NumberofPersistentMovementRelatedSpines{f} = sum(FieldData{f}.StatClass{1}.OverallMovementSpines(FieldData{f}.StatClass{end}.OverallMovementSpines));
        ...
        switch AnalysisType
            ...
            case 'Ignore'
                OtherSpineActivity = FieldData{f}.CalciumData{end}.OverallSpineActivity(AllOtherSpines, :);
                eval(['OtherSpineActivityAligned = ', currentanimal, '_Aligned{', num2str(latesession),'}.BinarizedOverallSpineData([', num2str(AllOtherSpines), '],:);'])
        end
                ...
                for ns = 1:length(NewSpines{f})             %%% FOR EACH NEW SPINE THAT IS ON A DENDRITE WITH AT LEAST ONE MOVEMENT RELATED SPINE
                    ...
                    switch AnalysisType
                        ...
                        case 'Ignore'
                            ...
                            eval(['NewSpineActivityAligned = ', currentanimal, '_Aligned{', num2str(latesession),'}.BinarizedOverallSpineData(', num2str(NewSpines{f}(ns)), ',:);'])
                    end
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                         end
                        ...
                        switch AnalysisType
                            case 'Subtract'
                                eval(['MoveSpineActivityAligned = ', currentanimal, '_Aligned{',num2str(latesession),'}.DendSubSynapseOnlyBinarized(', num2str(MovementSpinestoCompare(ms)), ',:);'])
                                eval(['AllOtherSpineActivityAligned = ', currentanimal, '_Aligned{', num2str(latesession), '}.DendSubSynapseOnlyBinarized([', num2str(setdiff(1:NumberofLateSpines, union(NewSpines{f}(ns), MovementSpinestoCompare(ms)))), '],:);'])
                            case 'Exclude'
                                eval(['MoveSpineActivityAligned = ', currentanimal, '_Aligned{',num2str(latesession),'}.SynapseOnlyBinarized(', num2str(MovementSpinestoCompare(ms)), ',:);'])
                                eval(['AllOtherSpineActivityAligned = ', currentanimal, '_Aligned{', num2str(latesession), '}.SynapseOnlyBinarized([', num2str(setdiff(1:NumberofLateSpines, union(NewSpines{f}(ns), MovementSpinestoCompare(ms)))), '],:);'])
                            case 'Ignore'
                                eval(['MoveSpineActivityAligned = ', currentanimal, '_Aligned{',num2str(latesession),'}.BinarizedOverallSpineData(', num2str(MovementSpinestoCompare(ms)), ',:);'])
                                eval(['AllOtherSpineActivityAligned = ', currentanimal, '_Aligned{', num2str(latesession), '}.BinarizedOverallSpineData([', num2str(setdiff(1:NumberofLateSpines, union(NewSpines{f}(ns), MovementSpinestoCompare(ms)))), '],:);'])
                        end
                        ...
                        if NewSpinestoEarlyMovementSpines(1,count)<clusterdistance
                            ...
                            if ismember(MovementSpinestoCompare(ms), AllMovementSpinesOnLateSession)
                            ...
                            for spf = 1:size(AllOtherSpineActivityAligned,1)
                                OtherSpFreq(1,spf) = numel(find(diff(AllOtherSpineActivityAligned(spf,:))>0))/(length(AllOtherSpineActivityAligned)/(ImagingFrequency*60));
                            end
                            NewSpFreq = numel(find(diff(NewSpineActivityAligned))>0)/(length(NewSpineActivityAligned)/(ImagingFrequency*60));
                                [~, CompSpFreqMatchedtoNS] = min(abs(OtherSpFreq-NewSpFreq));
                            CMRSFreq = numel(find(diff(MoveSpineActivityAligned))>0)/(length(MoveSpineActivityAligned)/(ImagingFrequency*60));   %%% Clustered MRS frequency
                            ...
                            switch AnalysisType
                                case 'Subtract'
                                    MoveSpineActivity = FieldData{f}.CalciumData{end}.SynapseOnlyBinarized_DendriteSubtracted(MovementSpinestoCompare(ms),:);
                                case 'Exclude'
                                    MoveSpineActivity = FieldData{f}.CalciumData{end}.SynapseOnlyBinarized(MovementSpinestoCompare(ms),:);
                                case 'Ignore'
                                    MoveSpineActivity = FieldData{f}.CalciumData{end}.OverallSpineActivity(MovementSpinestoCompare(ms),:);
                            end
                            ...
                            if success_centric_cluster_correlation >0.1
                                disp(['Animal ', currentanimal, ', Field ', num2str(f), ' spines ', num2str(NewSpines{f}(ns)), ' & ', num2str(MovementSpinestoCompare(ms)), ' have high noise correlation!'])
                            end
                            %%%
                            ...
                            if ~isempty(framesofinterest)
                                MovementswithClusterCoActivity = ExtractMovementswithKnownBounds(levertraceLate, framesofinterest, rewardperiodsLate, TimingValues, ImagingFrequency);
                            ...
                            if ~isempty(framesofinterest)
                                [SuccessfulPresseswithoutCoactivity] = ExtractMovementswithKnownBounds(levertraceLate, framesofinterest, rewardperiodsLate, TimingValues, ImagingFrequency);
                            ...
                            if ~isempty(framesofinterest)
                                MovementswithCompCoActivity = ExtractMovementswithKnownBounds(levertraceLate, framesofinterest, rewardperiodsLate, TimingValues, ImagingFrequency);
                            ...
                            DistanceMatchedNonMRPartners = setdiff(DistanceMatchedNonMRPartners, union(union(AllMovementSpinesOnLateSession, MovementSpinestoCompare), NewSpines{f}));
                            ...
                            switch AnalysisType
                                ...
                                case 'Ignore'
                                    eval(['DistanceMatchedActivity = ', currentanimal, '_Aligned{', num2str(latesession), '}.BinarizedOverallSpineData([', num2str(DistanceMatchedNonMRPartners), '],:);'])
                            end
                            ...
                            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                            DistanceMatchedNonNewPartners = find(AllDendriteDistances{f}(MovementSpinestoCompare(ms), :)<=clusterdistance);
                            ...
                            switch AnalysisType
                                ...
                                case 'Ignore'
                                    eval(['DistanceMatchedActivity = ', currentanimal, '_Aligned{', num2str(latesession), '}.BinarizedOverallSpineData([', num2str(DistanceMatchedNonNewPartners), '],:);'])
                            end
                            ...
                            MovementSpineDistanceMatchedControlCorr(1,closecount) = nanmedian(AllLateSpineCorrelations(MovementSpinestoCompare(ms),DistanceMatchedNonNewPartners));
            ...
            for hcp = 1:length(NewSpineMaxInd) %%% Note: every new spine should should have a corresponding max correlated partner, so you can use "hcp" as an index for new spines;
                ...
                if NewSpinesMaxCorr{f}(hcp) >= 0.2
                    switch AnalysisType
                        ...
                        case 'Ignore'
                            eval(['NewSpineActivityAligned = ', currentanimal, '_Aligned{', num2str(latesession),'}.BinarizedOverallSpineData(', num2str(NewSpines{f}(hcp)), ',:).*', currentanimal, '_Aligned{', num2str(latesession), '}.BinarizedOverallSpineData(', num2str(NewSpines{f}(hcp)), ',:);'])
                            eval(['HCPActivityAligned = ', currentanimal, '_Aligned{',num2str(latesession),'}.BinarizedOverallSpineData(', num2str(NewSpineMaxInd(hcp)), ',:).*', currentanimal, '_Aligned{',num2str(latesession),'}.BinarizedOverallSpineData(', num2str(NewSpineMaxInd(hcp)), ',:);'])
                            eval(['AllOtherSpineActivityAligned = ', currentanimal, '_Aligned{', num2str(latesession), '}.BinarizedOverallSpineData([', num2str(setdiff(1:NumberofLateSpines, union(NewSpines{f}(hcp), NewSpineMaxInd(hcp)))), '],:);'])
                    end
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    %%%% Find spines with similar frequencies for best comparisons
                    OtherSpFreq = [];
                ...
                for es = 1:length(ElimSpines{f})
                    ...
                    switch AnalysisType
                        ...
                        case 'Ignore'
                            ...
                            eval(['ElimSpineActivityAligned = ', currentanimal, '_Aligned{', num2str(earlysession),'}.BinarizedOverallSpineData(', num2str(ElimSpines{f}(es)), ',:);'])
                    end
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    ...
                    for ms = 1:length(MovementSpinestoCompare) 
                        switch AnalysisType
                            case 'Subtract'
                                eval(['MoveSpineActivityAligned = ', currentanimal, '_Aligned{',num2str(earlysession),'}.DendSubSynapseOnlyBinarized(', num2str(MovementSpinestoCompare(ms)), ',:);'])
                                eval(['AllOtherSpineActivityAligned = ', currentanimal, '_Aligned{', num2str(earlysession), '}.DendSubSynapseOnlyBinarized([', num2str(setdiff(1:NumberofLateSpines, union(ElimSpines{f}(es), MovementSpinestoCompare(ms)))), '],:);'])
                            case 'Exclude'
                                eval(['MoveSpineActivityAligned = ', currentanimal, '_Aligned{',num2str(earlysession),'}.SynapseOnlyBinarized(', num2str(MovementSpinestoCompare(ms)), ',:);'])
                                eval(['AllOtherSpineActivityAligned = ', currentanimal, '_Aligned{', num2str(earlysession), '}.SynapseOnlyBinarized([', num2str(setdiff(1:NumberofLateSpines, union(ElimSpines{f}(es), MovementSpinestoCompare(ms)))), '],:);'])
                            case 'Ignore'
                                eval(['MoveSpineActivityAligned = ', currentanimal, '_Aligned{',num2str(earlysession),'}.BinarizedOverallSpineData(', num2str(MovementSpinestoCompare(ms)), ',:);'])
                                eval(['AllOtherSpineActivityAligned = ', currentanimal, '_Aligned{', num2str(earlysession), '}.BinarizedOverallSpineData([', num2str(setdiff(1:NumberofLateSpines, union(ElimSpines{f}(es), MovementSpinestoCompare(ms)))), '],:);'])
                        end
                        MoveSpineActivityAligned(isnan(MoveSpineActivityAligned)) = 0;
                        AllOtherSpineActivityAligned(isnan(AllOtherSpineActivityAligned)) = 0;
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        ...
                        if ElimSpinestoEarlyMovementSpines(1,count)<clusterdistance 
                            ...
                            if success_centric_anticluster_correlation >0.1
                                disp(['Animal ', currentanimal, ', Field ', num2str(f), ' spines ', num2str(ElimSpines{f}(es)), ' & ', num2str(MovementSpinestoCompare(ms)), ' have high noise correlation!'])
                            end
                            DistanceMatchedNonMRPartners = find(AllDendriteDistances{f}(ElimSpines{f}(es),:)<=clusterdistance);
                            DistanceMatchedNonMRPartners = setdiff(DistanceMatchedNonMRPartners, union(union(AllMovementSpinesOnLateSession, MovementSpinestoCompare), ElimSpines{f}));
                            switch AnalysisType
                                case 'Subtract'
                                ...
                                case 'Ignore'
                                    eval(['DistanceMatchedActivity = ', currentanimal, '_Aligned{', num2str(earlysession), '}.BinarizedOverallSpineData([', num2str(DistanceMatchedNonMRPartners), '],:);'])
                            end
                            ...
                            framesofinterest = frames_during_allperiods(CoActiveDuringMovement_Addresses);
                            if ~isempty(framesofinterest)
                                MovementswithAntiClusterCoActivity = ExtractMovementswithKnownBounds(levertraceEarly, framesofinterest, rewardperiodsEarly, TimingValues,ImagingFrequency);
                            ...
                            framesofinterest = frames_during_allperiods(setdiff(1:length(frames_during_allperiods),CoActiveDuringMovement_Addresses));
                            if ~isempty(framesofinterest)
                                [SuccessfulPresseswithoutCoactivity] = ExtractMovementswithKnownBounds(levertraceEarly, framesofinterest, rewardperiodsEarly,TimingValues,ImagingFrequency);
                                movementcorr = corrcoef([SuccessfulPresseswithoutCoactivity, ModelMovement']);
                                movementcorr(1:1+size(movementcorr,1):numel(movementcorr)) = nan;
                                thissessionmovementcorr = movementcorr(1:end-1, 1:end-1);
                                ...
                                OtherMovementCorrElimVersion(closecount) = NaN;
                                OtherCorrwithModelMovementElimVersion(closecount) = NaN;
                            end
                            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                            %%% Find the same features for freq-matched
                            %%% spine pairs
                            ...
                            framesofinterest = frames_during_allperiods(comp_CoActiveAddresses);
                            if ~isempty(framesofinterest)
                                MovementswithCompCoActivity = ExtractMovementswithKnownBounds(levertraceEarly, framesofinterest, rewardperiodsEarly,TimingValues,ImagingFrequency);
                                if size(MovementswithCompCoActivity,2)>1
                                    movementcorr = corrcoef([MovementswithCompCoActivity, ModelMovement']);
                                    movementcorr(1:1+size(movementcorr,1):numel(movementcorr)) = nan;
                                    thissessionmovementcorr = movementcorr(1:end-1, 1:end-1);
