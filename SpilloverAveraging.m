...
FilteredBoutonResponses = cell(1,length(varargin));
...
for file = 1:length(varargin)
    for bouton = 1:length(varargin{file}.EventAmp)
        ...
        FilteredBoutonResponses{file} = [FilteredBoutonResponses{file}, nanmean(varargin{file}.BoutonResponse{bouton}(largerresponses))];
...
for file = 1:length(varargin)
    ...
    plot(0:length(medianbyfile{file})-1,medianbyfile{file});
end

All = nanmean(vertcat(medianbyfile{:}),1);
plot(0:length(All)-1, All, 'k', 'linewidth', 2)
plot(0,nanmean(cell2mat(FilteredBoutonResponses)), 'or', 'markerfacecolor', 'r')
